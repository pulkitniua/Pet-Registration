...
import newData from "./data/pet.json"
...
const DesktopInbox = ({ tableConfig, filterComponent, ...props }) => {
  ...
  else if (newData?.PetRegistrationApplications?.length) {
    result = (
      <ApplicationTable
        t={t}
        data={newData?.PetRegistrationApplications}
        columns={columns}
  ...
  useEffect(()=> {
    ...
    newData && console.log("***newData: ", newData?.PetRegistrationApplications)
