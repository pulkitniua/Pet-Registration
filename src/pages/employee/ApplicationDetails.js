...
import jsonData from '../../components/data/pet.json';
...
const ApplicationDetails = () => {
  ...
  const [jsonAppDetailsToShow, setJsonAppDetailsToShow] = useState({});
 ...
 useEffect(()=> {
  const applicationsData = jsonData?.PetRegistrationApplications
  if(applicationsData?.length){
    const applicationData = applicationsData?.filter(app => app?.applicationNumber === propertyId)
   if (applicationData?.length === 1)  setJsonAppDetailsToShow(applicationData[0])
    console.log("applicationsData: ",applicationData)
    // (applicationData?.length === 1) && console.log("applicationData: ", applicationsData)
  }
 }, [propertyId])
  ...
  return (
    <div>
      ...
      <ApplicationDetailsTemplate
        ...
        applicationData={jsonAppDetailsToShow}
